...
import com.utilities.ActivityHelper;
...
public class TableListActivity extends ListActivity {
	...
	private void fillList() {
		try {
			...
			Stmt stmt = db.prepare("SELECT Name,Peoples,ST_AsText(Geometry) FROM Towns;");
		/*	Stmt stmt = db
	...
	private class TableListAdapter extends ArrayAdapter<TableInfo> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			TextView srid = (TextView) v.findViewById(R.id.srid);
			srid.setText(objects.get(position).getSrid());
			return v;
		}
	}
}
